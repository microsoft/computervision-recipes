...
def prepare_data_blocks(blocks):
	data = []
	for block in blocks:
		data.append(load_tf_img(block))
	blocks = []
	return data
...
def infer(model_name,model_weight,target_dir,save_out_dir,block_size=(256,256),batch_size=1):
	...
	for filename in tqdm(filelists):
		...
		in1_img = GetOverlappingBlocks(in_clr.copy(),M,N,part)
		prepared_data_blocks = prepare_data_blocks(in1_img)
		in1_img = []

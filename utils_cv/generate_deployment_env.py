...
def generate_yaml(directory: str, ref_filename: str,
    ...
    extracted_libraries = [depend for depend in yaml_content['dependencies']
    ...
    if any(isinstance(x, dict) for x in yaml_content['dependencies']):
        ...
        ind = [yaml_content['dependencies'].index(depend)
               for depend in yaml_content['dependencies']
               if isinstance(depend, dict)][0]
        extracted_libraries += [depend for depend in
                                yaml_content['dependencies'][ind]['pip']
                                if any(lib in depend
                                       for lib in needed_libraries)]

    # Check whether additional libraries are needed
    not_found = [lib for lib in needed_libraries

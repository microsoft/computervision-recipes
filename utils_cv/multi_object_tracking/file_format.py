...
def convert_vott_MOTxywh(vott_path, converted_path) -> None: 
    ...
    """
    df = pd.read_csv(os.path.join(vott_path, [f for f in os.listdir(vott_path) if f.endswith('.csv')][0])) 
    
    # Sort image column and order by frame number
    df["time"] = df["image"].apply(lambda x: x.split("=")[1].replace(".jpg",""))
    ...
    with open(file, 'w') as f:
        ...
        for i, row in df.iterrows(): #in format [frame number] [id number] [bbox left] [bbox top] [bbox width] [bbox height][confidence score][class][visibility]            
            print('%d,%d,%.5f,%.5f,%.5f,%.5f,-1,-1,-1' % (row["frame"] ,row["label_numbered"] ,row["xmin"] , row["ymin"], row["xmax"]-row["xmin"] , row["ymax"]-row["ymin"]), file=f)
    ...
    for file_i in os.listdir(vott_path):    
        if file_i.split(".")[-1] in ["jpg","jpeg","png", "tif"]:
            ...
            shutil.copy(os.path.join(vott_path, file_i), os.path.join(dest_path, img_filename))
    ...
    im_width, im_height = im = Image.open(os.path.join(vott_path,"single_vid","img1",img_filename)).size
